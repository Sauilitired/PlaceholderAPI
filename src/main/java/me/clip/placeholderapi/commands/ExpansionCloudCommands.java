    ...
    @Override public boolean onCommand(CommandSender s, Command c, String label, String[] args) {
        ...
        if (args.length == 1) {
            msg(s, "&bExpansion cloud commands", " ", "&b/papi ecloud status",
                ...
                "&fDownload an expansion from the ecloud", "&b/papi ecloud refresh",
                "&fFetch the most up to date list of expansions available.", "&b/papi ecloud clear",
        ...
        if (args[1].equalsIgnoreCase("info")) {
            ...
            if (!(s instanceof Player)) {
                msg(s, (expansion.shouldUpdate() ? "&e" : "") + expansion.getName() + " &8&m-- &r"
                    + expansion.getVersion().getUrl());
                return true;
            }
            ...
            msg(s, "&bExpansion&7: &f" + expansion.getName(),
                "&bAuthor: &f" + expansion.getAuthor(), "&bVerified: &f" + expansion.isVerified());

            // latest version
            JSONMessage latestVersion =
                JSONMessage.create(color("&bLatest version: &f" + expansion.getLatestVersion()));
            latestVersion.tooltip(color(
                "&bReleased: &f" + expansion.getTimeSinceLastUpdate() + "\n&bUpdate information: &f"
                    + expansion.getVersion().getReleaseNotes()));
            ...
            versions.suggestCommand(
                "/papi ecloud versioninfo " + expansion.getName() + " " + expansion
                    .getLatestVersion());
            versions.send(p);
        ...
        if (args[1].equalsIgnoreCase("versioninfo")) {
            ...
            if (version == null) {
                msg(s, "&cThe version specified does not exist for expansion: &f" + expansion
                    .getName());
                return true;
            }
        ...
        if (args[1].equalsIgnoreCase("placeholders")) {
            ...
            if (placeholders == null) {
                msg(s, "&cThe expansion: &f" + expansion.getName()
                    ...
                    "&7You should contact &f" + expansion.getAuthor()
        ...
        if (args[1].equalsIgnoreCase("list")) {
            ...
            if (!(s instanceof Player)) {
                ...
                for (Entry<Integer, CloudExpansion> expansion : ex.entrySet()) {
                    ...
                    msg(s, "&b" + (expansion.getKey() + 1) + "&7: " + (expansion.getValue()
                        .shouldUpdate() ?
                        "&6" :
        ...
        if (args[1].equalsIgnoreCase("download")) {
            ...
            PlaceholderExpansion loaded =
                plugin.getExpansionManager().getRegisteredExpansion(args[2]);
            ...
            if (args.length == 4) {
                ...
                if (expansion.getVersion(version) == null) {
                    msg(s,
                        "&cThe version you specified does not exist for &f" + expansion.getName());
